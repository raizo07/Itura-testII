...
const MobileSlider = () => {
  ...
  return (
    <div className="w-[90%] mx-auto overflow-hidden px-4 py-8 block lg:hidden md:hidden">
      <motion.div className="flex gap-4" transition={{ duration: 0.6 }}>
        {visibleCards.map((card) => (
          <div
            key={card.id}
            className={`w-[90%] flex-shrink-0 rounded-xl shadow-lg text-white flex items-center text-center text-lg font-semibold border py-6 px-4 ${themeColor}`}
          >
